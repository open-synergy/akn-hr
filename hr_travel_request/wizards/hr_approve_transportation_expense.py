...
class HrApproveTransportationExpense(models.TransientModel):
    _name = "hr.approve_transportation_expense"
    _description = "Approved Transportation Expense"
    ...
    line_ids = fields.One2many(
        string="Details",
        comodel_name="hr.approve_transportation_expense_line",
    ...
    def onchange_line_ids(self):
        result = []
        if self.request_id:
            for line in self.request_id.transportation_expense_ids:
                result.append((0, 0, {
                    "line_id": line.id,
...
class HrApproveTransportationExpenseLine(models.TransientModel):
    _name = "hr.approve_transportation_expense_line"
    _description = "Approved Transportation Expense Line"

    wizard_id = fields.Many2one(
        string="Wizard",
        comodel_name="hr.approve_transportation_expense",
        ondelete="cascade",
    )
    line_id = fields.Many2one(
        string="Trasportation Expense Line",
